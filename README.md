...
That's it. The script installs git (if needed), clones the repo, installs Node.js, configures the WiFi hotspot, and registers the systemd service — all in one shot.
...
```bash
sudo SSID="My Lights" PASSPHRASE="mypassword" bash <(curl -fsSL https://raw.githubusercontent.com/idlegravity/dmx-macro-pad/main/install.sh)
```
...
├── scripts/
│   ├── install.sh              # Full installer (Node.js + hotspot + service)
